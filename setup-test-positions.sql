...
-- Використайте цей скрипт в SQL Editor Supabase
...
-- Перевірка створеного дерева посад
SELECT 
  p.name as position,
  COUNT(DISTINCT ph.descendant_id) - 1 as subordinates_count,
  ph.depth
FROM positions p
LEFT JOIN position_hierarchy ph ON p.id = ph.ancestor_id
GROUP BY p.id, p.name, ph.depth
ORDER BY ph.depth NULLS FIRST, p.name;

-- Перевірка матеріалів по посадах
SELECT 
  p.name as position,
  COUNT(pm.material_id) as materials_count
FROM positions p
LEFT JOIN position_materials pm ON p.id = pm.position_id
